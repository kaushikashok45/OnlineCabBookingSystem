	...
	var html;
	 $.get('/com.tomcat_hello_world/GetTrips',function(responseText){
            if(responseText!=null){
                ...
                tripData=Handlebars.compile("<div id=\"profileIcon\"><img class=\"image\" src=\"./resources/images/profile.svg\"></div>"+"<div id=\"editProfile\"><h2 id=\"header\">Trip History</h2><table id=\"tripList\" frame=\"box\"><tr><th>Trip id</th><th>Cab id</th><th>Cab Type</th><th>From</th><th>To</th><th>Trip Distance</th><th>Trip Fare</th><th>Trip Status</th><th>Trip Starting time</th><th>Trip Ending time</th></tr>"+"{{#each parsedTrips}}<tr> <td>{{id}}</td> <td>{{cabid}}</td> <td>{{cabType}}</td> <td>{{src}}</td> <td>{{dest}}</td> <td>{{distance}}km</td> <td>&#8377;{{fare}}</td> <td>{{status}}</td> <td>{{timeCreated}}</td> <td>{{timeEnded}}</td></tr> {{/each}}"+"</table></div>");
